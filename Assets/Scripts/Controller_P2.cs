...
/// <summary>
/// Player 2 Controller - Uses second gamepad (Controller 2 map)
/// </summary>
[RequireComponent(typeof(Rigidbody2D))]
public class Controller_P2 : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        // CRITICAL: Disable Controller map to prevent conflicts
        inputActions.Controller.Disable();
        
        // PLAYER 2: Find ShanWan controller ONLY (not Xbox)
        var gamepads = Gamepad.all;
        Gamepad shanwanGamepad = null;
        
        foreach (var pad in gamepads) {
            string deviceName = pad.name.ToLower();
            Debug.Log($"[P2] Checking device: {pad.name}");
            
            // Look specifically for ShanWan device
            if (deviceName.Contains("shanwan")) {
                shanwanGamepad = pad;
                Debug.Log($"[P2] Found ShanWan controller: {pad.name}");
                break;
            }
        }
        
        if (shanwanGamepad != null) {
            inputActions.devices = new[] { shanwanGamepad };
            Debug.Log($"[P2] ✅ Assigned to ShanWan controller: {shanwanGamepad.name}");
        } else {
            Debug.LogWarning("[P2] ⚠️ No ShanWan controller found, using keyboard fallback");
        }
    ...
    void OnEnable()
    {
        ...
        // Use Controller2 map for Player 2
    ...
    {
        bool gamepadMoving = inputActions.Controller2.Movement.IsPressed() || inputActions.Controller2.Movement1.IsPressed();
        ...
        bool gamepadThrottling = inputActions.Controller2.Throttle.IsPressed() || inputActions.Controller2.Reverse.IsPressed();
        ...
        if (Input.GetKeyDown(KeyCode.RightControl) && CanFire()) {
            Fire();
        }
    }
